    ...
    if request.method == "GET":
        ...
        if g.comp_date > 0:
            ...
            if start == 0 and end == 0:
                ...
                today_end = int(time.mktime(datetime.datetime(
                    today.year, today.month, today.day, 23, 59, 59).timetuple()))
                g.start = today_start
                g.end = today_end
            lt = time.localtime(g.start)
            day_start = int(time.mktime(datetime.datetime(
                lt.tm_year, lt.tm_mon, lt.tm_mday, 00, 00, 00).timetuple()))
