...
from rrd.model.template import Template
from rrd.model.strategy import Strategy
from rrd.model.action import Action
from rrd.model.grp_tpl import GrpTpl
...
def template_update_get(tpl_id):
    ...
    log.debug(t)
    return render_template('template/update.html', data={'tpl': t, 'ss': ss})
...
def template_view_get(tpl_id):
    ...
    ss = Strategy.select_vs(where='tpl_id = %s', params=[tpl_id], order='metric')
    t.action = Action.get(t.action_id)
    return render_template('template/view.html', data={'tpl': t, 'ss': ss})
